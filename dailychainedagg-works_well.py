...
Daily chained experimental price indices for monthly, weekly and daily indices. These are aggregated up
...
#import web scraped price data
x = pd.read_csv('.../data',encoding="latin_1")
...
def chaineddaily(data,idvar,classvar,datevar,pricevar):
    ...
    Daily = df['Daily']
    df1 = pd.DataFrame({"i" : range(0,len(date)),"period":date,"ChainedDaily":Chained,"Daily":Daily, "ons_item_number":classvalue[0]})
    df1.index = range(T)
    df1 = df1.fillna(100)
    return df1
...
results2.to_csv(".../daily_imputed_DC_item2.csv")

#aggregate
#%%
weights=pd.read_excel('/home/mint/my-data/Web scraped CPI/All prices test/Copy of weights-lager.xls')
def aggregation(indices,weights,frequency):
    data=pd.merge(indices,weights,left_on="ons_item_no",right_on="ons_item_no",how="outer")
...
Dail_Imputed=pd.read_csv(".../data/daily_imputed_DC_item2.csv")
Dail_Agg_imputed=aggregation(Dail_Imputed,weights,"daily")
Dail_Agg_imputed.to_csv(".../daily_imputed_DC_agg.csv")

#set up weekly indices. This was done using timestamp however, as the weeks run over the end of the year it caused a few problems. The code below fixes this but is a bit messy.
...
resultsav2.to_csv(".../daily_imputed_DC_item_average_prod_weekly.csv")
...
Resultsavmonth.to_csv(".../daily_imputed_DC_item_average_prod_monthly.csv")
...
Resultsavfort.to_csv(".../daily_imputed_DC_item_average_prod_fortnightly.csv")
...
resultsav21.to_csv(".../daily_imputed_DC_item_average_agg_weekly.csv")
...
Resultsavmonth1.to_csv(".../daily_imputed_DC_item_average_agg_monthly.csv")
...
Resultsavfort1.to_csv(".../daily_imputed_DC_item_average_agg_fortnightly.csv")
