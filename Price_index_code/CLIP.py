...
#    ward = DBSCAN(eps=2.2, algorithm = "kd_tree",leaf_size=3, min_samples=5).fit(data5)
    ward = MeanShift(bandwidth=None, seeds=None, bin_seeding=False, min_bin_freq=3, cluster_all=True, n_jobs=1).fit(data5)
...
#    print(dlen)
    ...
    dt = DecisionTreeClassifier(max_features = "log2", min_samples_leaf = 15)
    data4 = data3
    #criterion = "entropy",
        ...
        if bt.n_classes_ <=2:
            ...
            try:
                ...
                if date == basedate:
                    ...
                    basestructure.to_csv("/home/mint/my-data/Web_scraped_CPI/Code_upgrade/New_version/basestructure")
                    basedclust = applydecisiontree(data2,basedate,pd.read_csv("/home/mint/my-data/Web_scraped_CPI/Code_upgrade/New_version/basestructure"))
                    basedclust.to_csv("/home/mint/my-data/Web_scraped_CPI/Code_upgrade/New_version/basedclust")
                clust = applydecisiontree(data2,date,pd.read_csv("/home/mint/my-data/Web_scraped_CPI/Code_upgrade/New_version/basestructure"))
                new = pd.DataFrame()
                new = geo(clust, geobase(pd.read_csv("/home/mint/my-data/Web_scraped_CPI/Code_upgrade/New_version/basedclust")))
            #print("CLIP")
                bbb["type"] = "CLIP"
            except:
                new = jevons(pd.DataFrame(data),date,basedate)
            ...
            new = "wrong"
            print("wrong")
    ab= pd.read_csv("/home/mint/my-data/Web_scraped_CPI/Code_upgrade/New_version/metadata.csv")
    ab = ab.append(bbb)
    print(ab)
    del ab["Unnamed: 0"]
    ab.to_csv("/home/mint/my-data/Web_scraped_CPI/Code_upgrade/New_version/metadata.csv")
...
x["month"]=x["monthday"].apply(lambda x: int(str(x)[0:6]))
del x["monthday"]
...
def runthrough(data,basedate):
    ...
    os.remove("/home/mint/my-data/Web_scraped_CPI/Code_upgrade/New_version/basestructure")
    os.remove("/home/mint/my-data/Web_scraped_CPI/Code_upgrade/New_version/basedclust")
 
    return aa

aa=pd.DataFrame()
a = runthrough(x14, 201406)
a.to_csv("/home/mint/my-data/Web_scraped_CPI/Code_upgrade/data/CLIP2014.csv")

b = runthrough(x15, 201501)
b.to_csv("/home/mint/my-data/Web_scraped_CPI/Code_upgrade/data/CLIP2015.csv")

c = runthrough(x16, 201601)
c.to_csv("/home/mint/my-data/Web_scraped_CPI/Code_upgrade/data/CLIP2016.csv")
